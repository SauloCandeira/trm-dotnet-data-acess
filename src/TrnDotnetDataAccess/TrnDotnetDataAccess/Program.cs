    ...
    class Program
    {
        ...
        private static void ListarClientes()
        {
            ...
            while (sqlDataReader.Read())
            {
                ...
                cliente.Atualizar(sqlDataReader[1].ToString(), sqlDataReader[2].ToString());//converter de string para decimal/int
        ...
        private static void GravarNovoProduto()
        {
            ...
            var produto = new Produto("Sofa", 3500.00m, 1);
        ...
        private static void ListaProdutos()
        {
            ...
            var sqlCommand = new SqlCommand();
            sqlCommand.Connection = sqlConnection;
            sqlCommand.CommandText = "Select Id, Nome, PrecoUnitario, QuantidadeEstoque from Produtos";
            ...
            {
                Guid id = Guid.Parse(sqlDataReader[0].ToString());
                var produto = new Produto(id, sqlDataReader[1].ToString(), decimal.Parse(sqlDataReader[2].ToString()), int.Parse(sqlDataReader[3].ToString()));
            ...
            {
                Console.WriteLine($"Nome: {item.Nome} - Email: {item.PrecoUnitario}");
            }
        }
    }
}